...
enum RouteBuilder {
    static func roadRoute(
        ...
        guard let route = response.routes.first else {
            throw NSError(domain: "locbridge", code: 1, userInfo: [NSLocalizedDescriptionKey: "No route found"])
        }
...
enum GPXCodec {
    static func parse(_ url: URL) throws -> [CLLocationCoordinate2D] {
        ...
        guard !coords.isEmpty else {
            throw NSError(domain: "locbridge", code: 2, userInfo: [NSLocalizedDescriptionKey: "No track points found in GPX"])
        }
        return coords
    }

    static func export(_ coordinates: [CLLocationCoordinate2D], name: String = "locbridge Route") -> String {
        var body = """
        <?xml version="1.0" encoding="UTF-8"?>
        <gpx version="1.1" creator="locbridge" xmlns="http://www.topografix.com/GPX/1/1">
          <trk>
            <name>\(name)</name>
            <trkseg>
